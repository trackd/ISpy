...
Describe 'Expand-Type cmdlet' {
    It 'Completion_ExcludesCompilerGeneratedTypeNames' {
        ...
        #     Set-ItResult -Skipped -Because 'completers are not yet added to 5.1'
        #     return
        # }
    ...
    It 'Expand-Type_CustomSettings_DoesNotPostProcessOutput' {
        $settings = New-DecompilerSetting -UsingDeclarations
        $output = [System.IO.Directory]::Exists | Expand-Type -IncludeXml -Settings $settings

        $output | Should -Not -BeNullOrEmpty
        $output | Should -BeOfType System.String
        $output | Should -Match ([regex]::Escape('public static bool Exists('))
    }
    ...
    It 'Expand-Type_Default_OmitsXmlComments' {
        $output = [System.IO.Directory]::GetFiles | Expand-Type
        $output | Should -Not -BeNullOrEmpty
        $output | Should -BeOfType System.String
